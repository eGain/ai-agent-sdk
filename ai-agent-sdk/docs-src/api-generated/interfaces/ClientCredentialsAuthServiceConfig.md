[@egain/ai-agent-sdk API Reference - v0.2.3-beta.1](../README.md) / ClientCredentialsAuthServiceConfig

# Interface: ClientCredentialsAuthServiceConfig

Client credentials authentication configuration

## Table of contents

### Properties

- [type](ClientCredentialsAuthServiceConfig.md#type)
- [config](ClientCredentialsAuthServiceConfig.md#config)

## Properties

### type

• **type**: ``"client-credentials"``

#### Defined in

[core/auth/AuthenticationService.ts:64](https://github.com/eGainDev/ai-agent/blob/master/ai-agent-sdk/src/core/auth/AuthenticationService.ts#L64)

___

### config

• **config**: [`ClientCredentialsAuthConfig`](ClientCredentialsAuthConfig.md)

Client credentials configuration options

#### Defined in

[core/auth/AuthenticationService.ts:68](https://github.com/eGainDev/ai-agent/blob/master/ai-agent-sdk/src/core/auth/AuthenticationService.ts#L68)
