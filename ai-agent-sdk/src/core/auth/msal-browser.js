        ...
        monitorPopupForHash(popupWindow, popupWindowParent) {
            return new Promise((resolve, reject) => {
                ...
                const intervalId = setInterval(() => {
                    ...
                    // eGain Authorization server will be on same domain so check for redirectURI
                    const isValidHref = href && href !== "about:blank";
                    const allowedPaths = [
                        "auth-redirect",
                        "templates/chat/blank"
                    ];

                    let isRedirectPath = false;
                    isValidHref && allowedPaths.forEach(path => {
                        if (new URL(href)?.pathname?.indexOf(path) !== -1) {
                            isRedirectPath = true;
                        }
                    });

                    if (!isValidHref || !isRedirectPath) {
                        return;
                    }
                    clearInterval(intervalId);
                    let responseString = "";
